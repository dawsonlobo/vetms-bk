...
function getObjectIdFromDate(date) {
    var objectId = Math.floor(date.getTime() / 1000).toString(16) + "0000000000000000";
    return objectId;
}
...
function getImageFormat(buffer) {
    ...
    for (var _i = 0, formatSignatures_1 = formatSignatures; _i < formatSignatures_1.length; _i++) {
        var _a = formatSignatures_1[_i], signature = _a.signature, format = _a.format;
        if (signature.every(function (value, index) { return value === buffer[index]; })) {
            return format;
        }
    }
}
