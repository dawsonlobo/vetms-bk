...
export const getAll = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const totalCount = await FollowUp.countDocuments(query);
    ...
    const tableData = await FollowUp.find(query, finalProjection)
      .sort(sort)
...
export const getOne = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const appointment = await FollowUp.findOne({ _id: id, isDeleted: false }, projectionFields);

        if (!appointment) {
            res.status(404).json({ status: 404, message: "Followup not found or deleted" });
            return;
        }
